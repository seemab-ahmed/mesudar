...
//     ...prev,
//     [`${subCat}-${value}`]: true // <-- check new task by default
//   }));
...
//     const taskName = updated[catTitle][subCat][taskIdx];
//     updated[catTitle][subCat] = updated[catTitle][subCat].filter(
//       (_, idx) => idx !== taskIdx
//     );
//     return updated;
//   });

//   // Remove checked state for deleted task
//   setCheckedItems(prev => {
//     const newCheckedItems = { ...prev };
//     const catTitle = selectedCategory.categoryTitle;
//     const taskName = localTasksData[catTitle][subCat][taskIdx];
//     delete newCheckedItems[`${subCat}-${taskName}`];
...
//   // Move checked state from old task name to new task name
//   setCheckedItems(prev => {
//     const checked = prev[`${subCat}-${oldTask}`];
//     const newCheckedItems = { ...prev };
//     delete newCheckedItems[`${subCat}-${oldTask}`];
//     newCheckedItems[`${subCat}-${newTask}`] = checked;
//     return newCheckedItems;
//   });
...
export const EditTasksStep = ({
  ...
  const [editingTask, setEditingTask] = useState({ subCat: '', idx: -1 });
  const [editingTaskValue, setEditingTaskValue] = useState('');
  const [rearrangeState, setRearrangeState] = useState({}); // { [subCat]: false }
  ...
  // Rearrange tasks in a subcategory
  const handleRearrangeTasks = (subCat, newList) => {
    setLocalTasksData(prev => {
      const updated = { ...prev };
      updated[selectedCategory.categoryTitle][subCat] = newList;
      return updated;
    });
  ...
  // Draggable item for tasks
  const TaskItem = ({ item, itemSelected, dragHandleProps, itemIndex, subCat }) => (
    <li
      className={`flex items-center mb-2 group bg-white px-2 py-1 rounded cursor-move ${itemSelected ? 'shadow border border-[#1f7333]' : ''}`}
      {...dragHandleProps}
    >
      <div className="w-4 h-4 mr-2 border-2 border-[#1f7333] rounded-[5px] flex items-center justify-center text-xs text-black"></div>
      {editingTask.subCat === subCat && editingTask.idx === itemIndex ? (
        <input
          type="text"
          value={editingTaskValue}
          onChange={e => setEditingTaskValue(e.target.value)}
          onBlur={() => saveEditTask(subCat, itemIndex, item)}
          onKeyDown={e => {
            if (e.key === 'Enter') saveEditTask(subCat, itemIndex, item);
            if (e.key === 'Escape') { setEditingTask({ subCat: '', idx: -1 }); setEditingTaskValue(''); }
          }}
          className="border-b border-[#1f7333] px-1 py-0.5 focus:outline-none focus:border-[#1f7333] bg-white text-base"
          autoFocus
        ...
        <span
          className="cursor-pointer hover:underline text-black text-base"
          onClick={() => startEditTask(subCat, itemIndex, item)}
          tabIndex={0}
          onKeyDown={e => { if (e.key === 'Enter') startEditTask(subCat, itemIndex, item); }}
        >
          {item}
        </span>
      )}
      <button
        className="ml-2 text-xs text-red-400 hover:text-red-600 opacity-0 group-hover:opacity-100 transition-opacity"
        onClick={() => handleRemoveTask(subCat, itemIndex)}
        type="button"
        title="Delete Task"
      >
        <TrashIcon />
          ...
          <div key={subCat} className="border border-[#1f7333] shadow-md p-6 rounded-2xl bg-white md:max-w-[80%] w-full mx-auto mb-4">
            <div className="flex items-center justify-between">
              {editingSubCat === subCat ? (
                ...
                <div className='flex justify-between w-full'>
                  <span
                  className="font-semibold text-lg text-[#1f7333] cursor-pointer hover:underline dis"
                  onClick={() => startEditSubCat(subCat)}
                  tabIndex={0}
                  onKeyDown={e => { if (e.key === 'Enter') startEditSubCat(subCat); }}
                >
              ...
              {!rearrangeState[subCat] ? (
                <button
                  className="text-[#1f7333] border border-[#1f7333] px-3 py-1 rounded hover:bg-[#1f7333] hover:text-white transition"
                  onClick={() => setRearrangeState(prev => ({ ...prev, [subCat]: true }))}
                  type="button"
                >
                  Rearrange Tasks
                </button>
                ...
                <button
                  className="text-[#1f7333] border border-[#1f7333] px-3 py-1 rounded hover:bg-[#1f7333] hover:text-white transition"
                  onClick={() => setRearrangeState(prev => ({ ...prev, [subCat]: false }))}
                  type="button"
                >
                  Done
                </button>
            ...
            <ul className="ml-2">
              {!rearrangeState[subCat] ? (
                ...
                <DraggableList
                  list={localTasksData[categoryTitle][subCat] || []}
                  itemKey={task => task}
                  template={({ item, itemSelected, dragHandleProps, itemIndex }) => (
                    <TaskItem
                      item={item}
                  ...
                  onMoveEnd={newList => handleRearrangeTasks(subCat, newList)}
