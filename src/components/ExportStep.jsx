...
import { Document, Paragraph, Packer, ImageRun, HorizontalPositionRelativeFrom, HorizontalPositionAlign } from "docx";
...
doc.addFont('/fonts/Raleway-SemiBold.ttf', 'Raleway', 'bold');
  ...
  const fileName = `${selectedCategory}-${timestamp}.pdf`;
  ...
  try {
    ...
    console.log('PDF stored in database successfully:', response.data);
  } catch (error) {
    console.error('Failed to store PDF in database:', error.response?.data || error.message);
...
// PropTypes validation would go here if needed
